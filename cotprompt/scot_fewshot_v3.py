...
def _get_few_shot_example_messages():
    ...
    (变更 v4: 这里的 CoT 步骤和输出 JSON 结构已更新)
    """
...
**Step 2: Object-Level Analysis (Grouping Elements)**
   ...
   - **Object Relationships & Spatial Semantics:**
...
**Step 3: High-Level Analysis (Holistic View)**
   ...
   - **Symmetry, Repetition & Construction:**
...
(变更 v4: JSON 结构已更新)
{
  "transformation_type": "The category of rule found (e.g., 'Element-Level', 'Object-Level', 'Decomposition-Selection', 'Construction').",
  "key_observations": "A brief summary of the key findings from the analysis steps that led to the rule.",
...
(变更 v4: 示例 JSON 已更新)
{
  "transformation_type": "Geometric-Operation",
  "key_observations": "Analyzed all 3 training pairs. The input and output grids are always 3x3. The output appears to be a horizontal mirror of the input. Verified this matches all samples.",
...
"""
    ...
    # (变更 v4: 助手 JSON 输出已更新，以匹配新格式)
    assistant_response = """
{
  "transformation_type": "Decomposition-Selection",
  "key_observations": "Identified a 'Decomposition-Selection' task. The 9x3 input is composed of three 3x3 blocks stacked vertically (B1, B2, B3). The output is always one of these blocks. Analyzed statistical properties (Sum, MaxElementFrequency) for each block. H1 (Max Sum) fails T0. H2 (Min Sum) fails T0. H3 (Max Freq) fails T0. H4 (Min Freq) fails T1. The rules are inconsistent across training data. However, the 'Max Sum' rule works for T2 and matches the Test case's properties (B1 has Max Sum 39 and Max Freq 6).",
...
def construct_prompt(d):
    ...
    (变更 v4: 这里的 System Prompt 和 CoT 步骤已更新)
...
"""
    ...
    # demonstrating the desired "Local-to-Global" ToT reasoning (v4 with new JSON output).
    ...
    current_puzzle_lines.append(
        "   - **Element Mapping:** How does an output element at `(r, c)` get its value? Is it from a corresponding input element `(r, c)` or `(r/2, c/2)`?")
    current_puzzle_lines.append("   - **Neighborhood & Positional Logic:**")
    current_puzzle_lines.append(
        "     - Does the value of an output element `(r, c)` depend on the *neighbors* of the input element `(r, c)`? (e.g., 'element becomes 1 if it has > 3 neighbors with value 2').")
    current_puzzle_lines.append(
        "     - Is the element's *absolute position* (e.g., 'at a corner', 'on an edge', 'in the center') important?")
    ...
    current_puzzle_lines.append(
        "   - **Object Fate:** What happens to each object? (e.g., moved, number changed, scaled, distorted, deleted, copied).")
    current_puzzle_lines.append("   - **Object Relationships & Spatial Semantics:**")
    current_puzzle_lines.append(
        "     - **Containment:** Is one object 'inside' or 'enclosed by' another?")
    current_puzzle_lines.append(
        "     - **Relative Position:** Is an object 'above', 'below', 'left of', or 'right of' another?")
    ...
    current_puzzle_lines.append(
        "   - **Decomposition-Selection Check:** Does the output look like an *exact copy* of a *part* (sub-grid, object, or block) from the input? (e.g., input is 9x3, output is 3x3). If YES, the task is to find the *selection rule* (e.g., 'select the block with the most 5s', 'select the block with the lowest sum').")
    current_puzzle_lines.append("   - **Geometric & Topological Operations:**")
    current_puzzle_lines.append(
        "     - **Global Geometry:** Is there a global transformation? (e.g., 'rotate 90 degrees', 'flip vertically', 'mirror horizontally').")
    current_puzzle_lines.append(
        "     - **Topological Changes:** Is the grid 'folded'? Are 'holes' being 'filled'?")
    current_puzzle_lines.append("   - **Symmetry, Repetition & Construction:**")
    current_puzzle_lines.append(
        "     - **Symmetry:** Does the output 'complete a symmetrical pattern' or 'remove asymmetry'?")
    current_puzzle_lines.append(
        "     - **Repetition:** Does the output 'tile' or 'repeat' a shape/pattern from the input?")
