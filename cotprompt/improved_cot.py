...
"""
    ...
    user_prompt_lines.append("\n让我们一步一步思考来找出规则：")
    user_prompt_lines.append("1. **分析样本：** 查看所有的输入-输出对。它们之间一致的变化是什么？(请按顺序尝试)")
    user_prompt_lines.append(
        "   - **A. 元素分析 (局部)：** 输入中特定位置 `(行, 列)` 的元素与输出有何关系？这种变化是基于它的邻居还是它的绝对位置？")
    user_prompt_lines.append(
        "   - **B. 颜色/模式匹配 (对象)：** 是否有颜色被添加、移除或改变？是否在输入中识别了特定的模式或形状，然后对其进行修改、复制或用它来创建输出？")
    user_prompt_lines.append(
        "   - **C. 几何/尺寸 (全局)：** 输出网格的尺寸是否不同？整个网格或其一部分是否被移动（平移）、旋转或翻转（水平、垂直或对角线）？")
    user_prompt_lines.append(
        "   - **D. 构造/算法 (复杂)：** (如果以上规则不适用) 输出是否是*从头构建*的？是否存在一个算法规则（例如，'将输入模式A平铺N次，N是输入的高度'）？")
