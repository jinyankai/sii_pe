...
def construct_prompt(d):
    """
    构造用于大语言模型的提示词 (思维链版本 - 仅输出答案)
    引导模型进行CoT思考，但只输出最终的网格。
...
"""
    ...
    user_prompt_lines.append("   - **数字：** 是否有数字被添加、移除或改变？是否有些数字保持不变？为什么这样变？")
    user_prompt_lines.append("   - *整体*：** 网格中是否可以进行分块，比如说一个块中数字或元素特征显著？")
