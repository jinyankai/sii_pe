...
"""
    ...
    for i, example in enumerate(d['train']):
        ...
        user_prompt_lines.append(f"Example {i + 1} Output:")
    ...
    user_prompt_lines.append("1. **Analyze Examples:** Look at all input-output pairs. What is the consistent change?")
    user_prompt_lines.append("   - **Dimensions:** Is the output grid size different from the input grid size?")
    user_prompt_lines.append("   - **Colors (Numbers):** Are any colors added, removed, or changed? Do some colors stay the same?")
    user_prompt_lines.append("   - **Geometric Operations:** Is the entire grid, or a part of it, moved (translated), rotated, or flipped (horizontally, vertically, or diagonally)?")
    user_prompt_lines.append(
    ...
    user_prompt_lines.append(
        "   - **Element Analysis:** How does an element at a specific position `(row, col)` in the input relate to an element at the same or a different position in the output? Is this change based on its neighbors or its absolute position?")
